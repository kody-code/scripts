    ...
    def init_driver(self):
        try:
            ...
            if self.driver_path:
                ...
                root_dir = os.path.dirname(os.path.dirname(current_dir))  # 新增：获取 src 的父目录
                
                if os.name == 'nt':
                    # 修正路径：根目录/lib/win/chromedriver.exe
                    driver_path = os.path.join(root_dir, "lib", "win", "chromedriver.exe")
                else:
                    # 修正路径：根目录/lib/chromedriver-linux64/chromedriver
                    driver_path = os.path.join(root_dir, "lib", "chromedriver-linux64", "chromedriver")
